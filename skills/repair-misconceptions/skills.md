---
skill_id: "repair-misconceptions"
...
- Student is confusing two specific sounds but has a coherent model
  otherwise — use `give-contrastive-hint` instead
...
- `give-contrastive-hint` fires when the student distinguishes two
...
- `diagnose-prerequisite-gaps` fires when the student is missing a
  concept they never learned. **This skill** fires when the student
  *has* learned the concept but internalized a wrong version.
- `escalate-hint-level-gradually` fires during a stuck moment within
